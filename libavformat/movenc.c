...
static int mov_write_audio_tag(AVFormatContext *s, AVIOContext *pb, MOVMuxContext *mov, MOVTrack *track)
{
    ...
    int nNeedFollowMOV = 0;
    if (track->mode == MODE_MP4 && track->par->codec_type == AVMEDIA_TYPE_AUDIO && (mov_get_lpcm_flags(track->par->codec_id) > 0)) {
        nNeedFollowMOV = 1;
    }
    
    if (track->mode == MODE_MOV || nNeedFollowMOV > 0) {
    ...
    if ((track->mode == MODE_MOV || (nNeedFollowMOV > 0)) && track->par->codec_type == AVMEDIA_TYPE_AUDIO
...
static unsigned int mov_find_codec_tag(AVFormatContext *s, MOVTrack *track)
{
    ...
    int nNeedFollowMOV = 0;
    if (track->par->codec_type == AVMEDIA_TYPE_AUDIO && (mov_get_lpcm_flags(track->par->codec_id) > 0)){
        nNeedFollowMOV = 1;
    }
    
    if (track->mode == MODE_MOV || (nNeedFollowMOV > 0 && track->mode == MODE_MP4)) {
