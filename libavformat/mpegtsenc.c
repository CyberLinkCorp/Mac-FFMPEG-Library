...
static int mpegts_write_pmt(AVFormatContext *s, MpegTSService *service)
{
    ...
    for (i = 0; i < s->nb_streams; i++) {
        ...
        switch (st->codecpar->codec_id) {
            ...
            {
                // special handle for LPCM otherwise FFMPEG Player cannot play
    ...
    for (i = 0; i < s->nb_streams; i++) {
        ...
        *q++ = stream_type;
        ...
        case AVMEDIA_TYPE_AUDIO:
            ...
            if (st->codecpar->codec_id==AV_CODEC_ID_PCM_S16BE){
                *q++=0x05;
                *q++=0x08;
                *q++=0x48; //'H'
                *q++=0x44; //'D'
        ...
        *q++      = 0x10 | ts_st->cc; // payload indicator + CC
        ...
        if (is_start) {
            ...
            *q++ = 0x01;
            ...
            if (st->codecpar->codec_type == AVMEDIA_TYPE_AUDIO && st->codecpar->codec_id == AV_CODEC_ID_PCM_S16BE){
                // for lpcm header
                lpcmheader_len = 4;
            }
            ...
            len = payload_size + header_len + 3 + lpcmheader_len;
